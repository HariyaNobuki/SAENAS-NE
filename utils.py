...
def archlist2archcode(arch):
    return np.array(arch).reshape(-1)

def build_train_sample(model_pool):
    X1,X2,Y = [],[],[]
    for i in range(len(model_pool)):
        for j in range(i,len(model_pool)):
            X1.append(model_pool[i][0])
            X2.append(model_pool[j][0])
            if model_pool[i][1]>=model_pool[j][1]:
                Y.append(1)
            else:
                Y.append(0)
...
class GraphDataset(Dataset):
    def __init__(self,adjs,features) -> None:
...
def to_oneshot(array,len_feature=16):
    array = np.array(array)
    matrix = np.full((len(array),len_feature),0)
