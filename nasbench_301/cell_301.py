...
from utils import featrue_extract_by_graph
...
class Cell301:
    ...
    def encode(self, predictor_encoding, nasbench=None, deterministic=True, cutoff=None):
        ...
        elif predictor_encoding == "seq":
            return self.encode_seq()
        elif predictor_encoding == 'gcn':
            return self.encode_gcn()
        else:
    ...
    def encode_seq(self):
        return np.array(self.arch).reshape(-1)
    
    def encode_g2v(self,searchSpace,g2v_model):
        edges = []
        features = {}
        matrix,ops = self.encode(predictor_encoding="gcn",nasbench=searchSpace.nasbench)
        hash_info = str(searchSpace.get_hash({"arch":self.arch}))
    ...
    def encode_gcn(self):
        normal_adj,normal_ops = self.build_mat_encoding(self.arch[0])
        reduc_adj,reduc_ops = self.build_mat_encoding(self.arch[1],is_normal=False)
        normal_adj = np.concatenate([normal_adj,np.zeros_like(normal_adj)],axis=1)
        reduc_adj = np.concatenate([np.zeros_like(reduc_adj),reduc_adj],axis=1)
        return np.concatenate([normal_adj,reduc_adj]),\
               np.concatenate([normal_ops,reduc_ops])


    def build_mat_encoding(self,normal,is_normal=True):
        adj = np.zeros((11, 11))
        ops = np.zeros((11,))
        ...
        ops[2]=prev_b0_o1+2+ops_offset
        ops[3]=prev_b0_o2+2+ops_offset
        ops[4]=prev_b1_o1+2+ops_offset
        ops[5]=prev_b1_o2+2+ops_offset
        ops[6]=prev_b2_o1+2+ops_offset
        ops[7]=prev_b2_o2+2+ops_offset
        ops[8]=prev_b3_o1+2+ops_offset
        ops[9]=prev_b3_o2+2+ops_offset
        return adj,ops
